...
class TakenAnimalControllerTest {
    ...
    @Autowired
    lateinit var repo: TakenAnimalRepo
    ...
    fun getListEmpty() {
        ...
        val response = mockMvc.perform(get("/taken/get"))
                .andExpect { status().isOk }
                .andReturn()

        val mapper = ObjectMapper()

        val animalsList = mapper.readValue(response.response.contentAsString, object : TypeReference<List<TakenAnimal>>() {})
    ...
    fun getListAfterDate() {
        ...
        var lostAnimal = TakenAnimal(id = UUID.randomUUID(), date = today)
        lostAnimal = repo.save(lostAnimal)

        val request = get("/taken/get").queryParam("dateStart", yesterday.toString())
    ...
    fun getListBeforeDate() {
        ...
        var lostAnimal = TakenAnimal(id = UUID.randomUUID(), date = tomorrow)
        lostAnimal = repo.save(lostAnimal)

        val request = get("/taken/get").queryParam("dateStart", today.toString())
        val response = mockMvc.perform(request)
                .andExpect { status().isOk }
                .andReturn()
    ...
    fun getListBetweenDatesDate() {
        ...
        val requestStartToday = get("/taken/get")
        ...
        val animalsListStartToday = mapper.readValue(response.response.contentAsString, object : TypeReference<List<TakenAnimal>>() {})
        Assertions.assertEquals(1, animalsListStartToday.size)
        Assertions.assertEquals(lostAnimal, animalsListStartToday[0])


        val requestEndToday = get("/taken/get")
                .queryParam("dateStart", yesterday.toString())
        ...
        val animalsListEndToday = mapper.readValue(response.response.contentAsString, object : TypeReference<List<TakenAnimal>>() {})
        Assertions.assertEquals(1, animalsListEndToday.size)
        Assertions.assertEquals(lostAnimal, animalsListEndToday[0])

        val requestTodayToday = get("/taken/get")
                .queryParam("dateStart", today.toString())
                .queryParam("dateEnd", today.toString())
        ...
        val animalsListTodayToday = mapper.readValue(response.response.contentAsString, object : TypeReference<List<TakenAnimal>>() {})
        Assertions.assertEquals(1, animalsList.size)
        Assertions.assertEquals(lostAnimal, animalsList[0])
    }
    ...
    fun getListBreed() {
        ...
        var lostAnimal = TakenAnimal(id = UUID.randomUUID(), animal = animal)
        lostAnimal = repo.save(lostAnimal)

        val request = get("/taken/get").queryParam("breed", breed)
        val response = mockMvc.perform(request)
                .andExpect { status().isOk }
                .andReturn()
    ...
    fun getListType() {
        ...
        val request = get("/taken/get").queryParam("type", type.name)
        val response = mockMvc.perform(request)
                .andExpect { status().isOk }
                .andReturn()
